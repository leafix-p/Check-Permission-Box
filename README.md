...
- **检查权限状态**：Switch 为 ON 表示已授权，OFF 表示未授权
- **申请权限流程**：用户将 Switch 从 OFF 拨向 ON → 自动发起权限申请
  - 申请成功 → Switch 保持 ON
  - 申请失败或被拒绝 → Switch 恢复为 OFF
- **版本兼容**：若当前设备系统版本低于权限的最低 API 要求，该权限的 Switch 自动**禁用**（不可交互）
- **状态刷新**：返回应用时通过 `onResume` 生命周期自动重新检查并刷新所有权限状态
